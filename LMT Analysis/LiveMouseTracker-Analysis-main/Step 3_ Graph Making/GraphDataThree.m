...
function GraphDataThree( Title, Legend, YLabel, YValuesWTs, Behaviours, norm_allmice_Genotype1, norm_allmice_Genotype2, norm_allmice_Genotype3, norm_allmice_Genotype1_err, norm_allmice_Genotype2_err, norm_allmice_Genotype3_err)
...
% represented as a grey region behind the animals
    ...
    b = bar(xValues, vertcat(norm_allmice_Genotype1, norm_allmice_Genotype2, norm_allmice_Genotype3), 'FaceAlpha', 0.5, 'EdgeColor', 'none');
    b(1).FaceColor = '#ff4343';
    b(2).FaceColor = '#edc211';
    b(3).FaceColor = '#edc251';
    drawnow;
    hold on;
    ...
    hold on
    err2 = errorbar(xValues+b(3).XOffset, norm_allmice_Genotype3, norm_allmice_Genotype3_err, 'k.')
    hold on
    
    % 4. Grey shaded region (std/sem of WT animals)
    X_pos_firstBar = xValues+b(1).XOffset
    X_pos_secondBar =xValues+b(2).XOffset
    X_pos_thirdBar =xValues+b(3).XOffset
    
    xValues = reshape ([ X_pos_firstBar ; X_pos_secondBar; X_pos_thirdBar], size(X_pos_firstBar,2), [] );
    xValues = xValues(:)';
    
    upper_line= repelem(YValuesWTs(2, :), 3); %%repeats these points so that the region behind each of the two bars has the same value (gives straight line)
    lower_line = repelem(YValuesWTs(3,:), 3);
