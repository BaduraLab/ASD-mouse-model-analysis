function[results_All_BehavType, results_All, results_animalData] = ProcessSQL_mouse(dbfile)
% This function processes the SQL file and organises the data relating to each mouse

% Connect to the file
conn            =       sqlite(dbfile);                        % connects to the file
...
results_quadratic             =   fetch(conn, sqlquery_quadraticEvents);
results_quadratic_BehavType   =   results_quadratic(:,1);
results_quadratic             =   cell2mat(results_quadratic(:, 2:end));


% Concatenate all data
results_All = vertcat(results_solo, results_dyadic, results_triadic, results_quadratic);
results_All_BehavType = vertcat(results_solo_BehavType, results_dyadic_BehavType, results_triadic_BehavType, results_quadratic_BehavType);
